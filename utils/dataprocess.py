...
def split_into_chunks(vid_names, seqlen, stride):
    ...
    # # 去除其中重复的元素，并按元素由大到小返回一个新的无元素重复的元组或者列表
    # # return_index=true 返回新列表元素在旧列表中的位置（下标），并以列表形式存储。
    # perm = np.argsort(group)  # 返回的是元素值从小到大排序后的索引值的数组
    # video_names, group = video_names[perm], group[perm]  # 排序
    #
    # indices = np.split(np.arange(0, vid_names.shape[0]), group[1:])  # 按group切分
    # # np.arange()函数返回一个有终点和起点的固定步长的排列，如[1, 2, 3, 4, 5]，起点是1，终点是6，步长为1。
    # for idx in range(len(video_names)):
    #     indexes = indices[idx]  # 对切分后的每一段分别处理
    ...
    # 但是直接取不重复数组会出错，因为不重复数组的序号不完全连续
    video_names = np.arange(0, vid_names.shape[0])
    chunks = view_as_windows(video_names, (seqlen,), step=stride)
    start_finish = chunks[:, (0, -1)].tolist()
    video_start_end_indices += start_finish
