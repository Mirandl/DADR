...
from model.spin_geometry import rotation_matrix_to_angle_axis, rot6d_to_rotmat
from model.smpl import SMPL, SMPL_MODEL_DIR, H36M_TO_J14, SMPL_MEAN_PARAMS
...
class Regressor(nn.Module):
    ...
    def forward(self, x, init_pose=None, init_shape=None, init_cam=None, n_iter=3, J_regressor=None):
        batch_size = x.shape[0] # 在这里重新定义batch_size!
        ...
        pred_pose = init_pose  # [8,144]
        pred_shape = init_shape  # [8,10]
        pred_cam = init_cam  # [8,3]
        for i in range(n_iter):
...
def get_pretrained_hmr():
    device = 'cuda'
    model = hmr().to(device)
    checkpoint = torch.load(osp.join('/home/neu307/liumeilin/datasets/vibe_data', 'spin_model_checkpoint.pth.tar'))
